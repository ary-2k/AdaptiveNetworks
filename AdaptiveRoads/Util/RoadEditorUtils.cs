...
internal static class RoadEditorUtils {
    ...
    public static void OnDPTMoreOptions(UICustomControl dpt) {
        ...
        if(target is NetLaneProps netLaneProps
            ...
            panel.AddButton("Duplicate" + strAll, null, delegate () {
                AddTransitionProps(groupPanel, cloned_items.ToArray());
            });
            panel.AddButton("Copy" + strAll, null, delegate () {
                ClipBoard.SetData(original_items);
            });
    ...
    #region TransitionProp
    public static void AddTransitionProps(
        RoadEditorCollapsiblePanel groupPanel,
        TransitionProp[] props,
        Track sourceTrack = null) {
        try {
            NetInfo sourceInfo = sourceTrack?.ParentInfo;
            Log.Called("props.count=" + props.Length, sourceInfo, sourceTrack);

            if (props == null || props.Length == 0) return;
            TransitionProp[] m_props = groupPanel.GetArray() as TransitionProp[];
            var m_props2 = m_props.AddRangeToArray(props);
            var targetTrack = groupPanel.GetTarget() as Track;
            ...
            foreach (var prop in props) {
                sidePanel.AddToggle(groupPanel, prop, arrayField, targetTrack);
            }
